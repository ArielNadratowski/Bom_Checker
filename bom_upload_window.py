...
class BomUploadWindow(Toplevel):
    ...
    def create_select_file_button(self, bom_storage, bom_frame, main_window):
        ...
        select_file_button = ButtonWithLabel(self, 
                                            ...
                                            'Select File', 
                                            select_file_row, 
                                            command_ = lambda: self.import_bom(bom_storage, bom_frame, main_window), # bom object gets created here
                                            column_span = 7, 
                                            sticky_ = "W")
    ...
    def import_bom(self, bom_storage, bom_frame, main_window): 
        bom_dataframe, status = self.select_and_load_bom()
        cleaned_bom = self.make_cleaned_bom(bom_dataframe)
        bom = self.make_bom_object(main_window, cleaned_bom, status, bom_storage)
    ...
    def select_and_load_bom(self):
        ...
        bom_dataframe, status = self.load_bom_from_file(filename, header)
        return bom_dataframe, status
    ...
    def make_cleaned_bom(self, bom_dataframe):
        ...
        cleaned_bom = self.clean_bom(bom_dataframe, input_ref_dsg, input_description, input_quantity, input_manufacturer, input_manufacturer_part_number)
        return cleaned_bom

    def make_bom_object(self, main_window, cleaned_bom, status, bom_storage):
        bom = Bom(main_window, self, cleaned_bom, status)
        bom_storage.append(bom)
        return bom

    def make_bom_checkbuttons(self, bom_frame,bom):
        check_button = tk.Checkbutton(bom_frame, text = bom.name, variable = bom.selected)
        check_button.pack()

    def load_bom_from_file(self, filename, header):
    ...
    def clean_bom(self, input_bom, input_ref_dsg, input_description, input_quantity, input_manufacturer, input_manufacturer_part_number):
        ...
        input_bom_no_na[input_manufacturer_part_number] = input_bom_no_na[input_manufacturer_part_number].astype(str)
